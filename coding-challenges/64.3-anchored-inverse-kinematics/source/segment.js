class Segment {
  static createRoot(start, length, angle) {
    const root = new Segment(start, null, length, angle)
    root.initialStart = start
    return root
  }
  ...
  follow() {
    if (this.child === null && (0 >= mouseX || mouseX >= width || 0 >= mouseY || mouseY >= height)) {
      return
    }
  ...
  anchor() {
    this.start = (this.parent !== null ? this.parent.end : this.initialStart)
  }
}